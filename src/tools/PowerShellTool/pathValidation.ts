...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .miniClaude is a dangerous directory
  // and internal editable paths live under ~/.miniClaude/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.miniClaude; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.miniClaude/settings.json (Claude's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.miniClaude`, validate subsequent statements with
  // cwd='./.miniClaude'. This would be more permissive but requires careful
  // handling of:
