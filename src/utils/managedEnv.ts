 ...
 * env vars that the remote's ~/.miniClaude settings.env MUST NOT clobber (see
 ...
 * user's ~/.miniClaude/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.miniClaude/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.miniClaude.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.miniClaude/settings.json env (gh#217). policy/flag
