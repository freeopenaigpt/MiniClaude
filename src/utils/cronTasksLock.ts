// Scheduler lease lock for .miniClaude/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.miniClaude', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .miniClaude/ doesn't exist yet — create it and retry once. In steady
