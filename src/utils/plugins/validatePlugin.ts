...
function detectManifestType(
  ...
  // Check if it's in .miniClaude-plugin directory
  if (dirName === '.miniClaude-plugin') {
 ...
 * '..' a user added to "climb out of .miniClaude-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .miniClaude-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .miniClaude-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .miniClaude-plugin/), not relative to marketplace.json. ' +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.miniClaude-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.miniClaude-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .miniClaude-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.miniClaude-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .miniClaude-plugin/marketplace.json or .miniClaude-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
