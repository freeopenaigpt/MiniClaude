...
/**
 * If filePath is inside a .miniClaude/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .miniClaude/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.miniClaude', 'skills')),
      prefix: '/.miniClaude/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.miniClaude', 'skills')),
      prefix: '~/.miniClaude/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.miniClaude/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.miniClaude/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.miniClaude${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.miniClaude${sep}settings.local.json`)
  ) {
    // Include .miniClaude/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .miniClaude/commands or .miniClaude/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.miniClaude', 'commands')
  const agentsDir = join(getOriginalCwd(), '.miniClaude', 'agents')
  const skillsDir = join(getOriginalCwd(), '.miniClaude', 'skills')
 ...
 * Path format: ~/.miniClaude/projects/{sanitized-cwd}/...
 */
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .miniClaude/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .miniClaude
      // segment when it's followed by 'worktrees'. Any nested .miniClaude directories
      ...
      if (dir === '.miniClaude') {
        ...
        if (
          ...
          break // Skip this .miniClaude, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .miniClaude , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .miniClaude , .bashrc..., settings.json.
 ...
 * - Claude config files (.miniClaude/settings.json, .miniClaude/commands/, .miniClaude/agents/)
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .miniClaude/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .miniClaude is a dangerous directory
  ...
  // 1.6. Check for .miniClaude/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .miniClaude/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .miniClaude/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.miniClaude) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .miniClaude/ (project or global).
    // Accepts both the broad patterns ('/.miniClaude/**', '~/.miniClaude/**') and
    // narrowed ones like '/.miniClaude/skills/my-skill/**' so users can grant
    ...
    // '/.miniClaude/../**' from leaking this bypass outside .miniClaude/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .miniClaude/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.miniClaude/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.miniClaude/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.miniClaude is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.miniClaude/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .miniClaude/launch.json — desktop preview config (dev server command + port).
  ...
  // .miniClaude/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .miniClaude/ only (not ~/.miniClaude/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.miniClaude', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.miniClaude/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.miniClaude/teams/) for swarm coordination
