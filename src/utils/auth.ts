 ...
 * to the user's ~/.miniClaude/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.miniClaude settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
 ...
 * from ~/.miniClaude/settings.json or project settings is ignored.
 */
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  return { success: false, warning: 'OAuth is not supported' }
}

export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  // OAuth functionality removed - always return null
  return null
})
...
export function saveCodexOAuthTokens(tokens: CodexTokens): void {
  // OAuth functionality removed - no-op
}
...
export function handleOAuth401Error(
  ...
  // OAuth functionality removed - always return false
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.miniClaude.json is user-writable and cannot be trusted.
