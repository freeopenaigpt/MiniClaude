...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.miniClaude/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.miniClaude/settings.json)'
    case 'projectSettings':
      return 'Project settings (.miniClaude/settings.json)'
    case 'localSettings':
      return 'Local settings (.miniClaude/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.miniClaude/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.miniClaude/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
