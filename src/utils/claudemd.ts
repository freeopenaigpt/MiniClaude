 ...
 * 2. User memory (~/.miniClaude/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (CLAUDE.md, .miniClaude/CLAUDE.md, and .miniClaude/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .miniClaude/CLAUDE.md, and all .md files in .miniClaude/rules/ are checked in each directory for Project memory
 *
...
/**
 * Processes all .md files in the .miniClaude/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .miniClaude/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.miniClaude/rules/*.md files
    ...
    // .miniClaude/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like CLAUDE.md and .miniClaude/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .miniClaude/CLAUDE.md (Project)
        const dotClaudePath = join(dir, '.miniClaude', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .miniClaude/rules/*.md files (Project)
        const rulesDir = join(dir, '.miniClaude', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .miniClaude/CLAUDE.md from the additional directory
        const dotClaudePath = join(dir, '.miniClaude', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .miniClaude/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.miniClaude', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .miniClaude/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .miniClaude/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .miniClaude/CLAUDE.md)
  ...
  // Process project unconditional .miniClaude/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .miniClaude/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .miniClaude
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .miniClaude
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .miniClaude/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .miniClaude/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.miniClaude${sep}rules${sep}`)
  ) {
    return true
  }
