 ...
 * user-controlled files (~/.miniClaude/settings.json and ~/.miniClaude.json),
 ...
 *
 * We read from global config (~/.miniClaude.json) and user settings
 * (~/.miniClaude/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.miniClaude/settings.json),
