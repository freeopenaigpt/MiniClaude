// Scheduled prompts, stored in <project>/.miniClaude/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.miniClaude', 'scheduled_tasks.json')
...
/**
 * Read and parse .miniClaude/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .miniClaude/scheduled_tasks.json with the given tasks. Creates .miniClaude/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.miniClaude'), { recursive: true })
 ...
 * written to .miniClaude/scheduled_tasks.json and dies with the process. The
