 ...
 * stopping there makes the parent project's `.miniClaude/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.miniClaude/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .miniClaude directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.miniClaude/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .miniClaude/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.miniClaude', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.miniClaude', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .miniClaude/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .miniClaude/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .miniClaude/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.miniClaude is
 ...
 * @param dir Directory (eg. "~/.miniClaude/commands")
 * @returns Array of parsed markdown files with metadata
 */
