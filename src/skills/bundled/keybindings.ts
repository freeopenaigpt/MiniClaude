...
const FILE_FORMAT_EXAMPLE: KeybindingsSchemaType = {
  ...
  $docs: 'https://code.miniClaude.com/docs/en/keybindings',
  bindings: [
    {
      context: 'Chat',
      bindings: {
...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.miniClaude/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.miniClaude/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.miniClaude/keybindings.json`.',
  '',
  ...
  'Location: ~/.miniClaude/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.miniClaude/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
